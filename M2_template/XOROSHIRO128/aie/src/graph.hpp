    ...
    main_loop:
    {
        for (int col = 0; col < COLS; col++)
        {
            for (unsigned cu = 0; cu < CU_PER_COL; cu++)
            {
                    ...
                    krnl_rand[col][cu] = kernel::create(xoroshiro128ppmvect);
                    krnl_icdf[col][cu] = kernel::create(subsequent);
                    // Sourcing the kernels
                    source(krnl_rand[col][cu]) = "xoroshiro128++-vect.cpp";
                    source(krnl_icdf[col][cu]) = "subsequent.cpp";

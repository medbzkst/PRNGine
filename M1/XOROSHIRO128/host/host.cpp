...
static uint64_t s[2] = {0x0123456789ABCDEF, 0xFEDCBA9876543210};
...
int main(int argc, char **argv)
{
    ...
    uint64_t val = 0;
    std::chrono::duration<double> cpu_duration = std::chrono::nanoseconds::zero();
    auto cpu_start = std::chrono::high_resolution_clock::now();
    for (auto runs = 0; runs < size_out * 40 * 8; runs++)
    {
        val ^= next();
    }
    ...
    std::cout << "Exec time on CPU : " << cpu_duration.count() << " s " << std::endl;

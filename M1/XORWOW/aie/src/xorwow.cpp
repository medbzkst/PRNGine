...
void xorwow(input_window<int32> *restrict states, output_pktstream *restrict result)
{
    ...
    int len = window_readincr(states);

    aie::vector<int32, 4> s[5];

    s[0] = window_readincr_v<4>(states);
    s[1] = window_readincr_v<4>(states);
    s[2] = window_readincr_v<4>(states);
    s[3] = window_readincr_v<4>(states);
    s[4] = window_readincr_v<4>(states);
    int32 cnt = 0;
    ...
    for (int b = 0; b < bound; b++)
        chess_prepare_for_pipelining
        {
            ...
            writeincr(result, value[0]);
            writeincr(result, value[1]);
            writeincr(result, value[2]);
            writeincr(result, value[3], b == bound - 1);

...
void sfmt(input_window<int32> *restrict states, output_pktstream *restrict result)
{
    ...
    uint32 ID = getPacketid(result, 0); // for output pktstream
    writeHeader(result, pktType, ID);   // Generate header for output

    const int32 len = window_readincr(states);

    alignas(aie::vector_decl_align) int32 s[N * 4];

    for (int i = 0; i < 4 * N; i++)
        s[i] = window_readincr(states); // _INIT_STATE_BUFF_
    ...
    for (int ii = 0; ii < bound; ii++)
        chess_prepare_for_pipelining
        {
            ...
            writeincr(result, r2[0]);                                  // Write the result to the output stream
            writeincr(result, r2[1]);                                  // Write the result to the output stream
            writeincr(result, r2[2]);                                  // Write the result to the output stream
            writeincr(result, r2[3], ii == bound - 1);                 // Write the result to the output stream
